...
class ObservableDynamics(ABC):
    ...
    def make_observable_dynamics(self):
        ...
        class ObservableSystem(SystemDynamics):
            ...
            def forward(inner_self, y, u, t):
                x = self.invert_observation(y)
                dydx = self.obs_jacobian(x)
                return (dydx @ self(x, u, t)[..., None]).squeeze(-1)

        return ObservableSystem()

    def obs_jacobian(self, observations):
